...
function checkAll(){
	if(!checkEmail($("#email1").val(), $("#email2").val())){
		return false;
...
%>
	<div id="logo">
		<a id="logoa" href="../main/main.jsp" >YOMOZOMO</a>
	</div>
...
<form action="modifyOk.jsp" method="post" onsubmit="return checkAll()">
	...
	<h6>주소</h6>
	<div class="input-group mb-3">
		<input type="text" id="zipcode" name="zipcode" value="<%=vo.getZIPCODE() %>" class="form-control" placeholder="우편번호" aria-label="Recipient's username" aria-describedby="button-addon2" readonly>
  		<button class="btn btn-outline-secondary" type="button" id="searchAddrs">주소찾기</button>
  	</div>
  	<div class="input-group mb-3">
  		<input type="text" id="address" name="address" value="<%=vo.getADDRESS() %>" class="form-control" aria-label="Text input with dropdown button" placeholder="주소입력" readonly>
  	</div>
  	<div class="input-group mb-3">
  		<input type="text" id="addrsDetail" name="addrsDetail" value="<%=vo.getADDRESSDETAIL() %>" class="form-control" aria-label="Text input with dropdown button" placeholder="상세주소 입력">
	</div>
	<div id="button">
